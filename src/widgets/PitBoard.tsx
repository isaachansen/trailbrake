...
function PitBoard({ theme, config }: BaseWidgetProps<PitBoardConfig>) {
  ...
  const cell = (label: string, value: React.ReactNode, color: string) => (
    <div style={{ padding: "7px 11px", background: t.cell, borderRadius: 9, minWidth: 0 }}>
      <div style={{ fontFamily: theme.font.label, fontSize: "0.5em", fontWeight: 600, letterSpacing: "0.1em", color: t.textDim2 }}>{label}</div>
  ...
  const corner = (label: string) => (
    <div style={{ flex: 1, textAlign: "center", padding: "5px 0", background: t.cell, borderRadius: 7 }}>
      <div style={{ fontFamily: theme.font.label, fontSize: "0.46em", fontWeight: 600, letterSpacing: "0.08em", color: t.textDim2 }}>{label}</div>
    ...
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", color: t.text, padding: theme.widgetPad, boxSizing: "border-box", overflow: "hidden" }}>
        ...
        return (
          <div style={{ padding: "7px 13px", borderRadius: 11, background: stopNeeded ? "rgba(255,180,61,0.14)" : t.cell, border: stopNeeded ? `1px solid rgba(255,180,61,0.35)` : `1px solid ${t.surfaceBorder}`, display: "flex", alignItems: "center", gap: 9 }}>
            ...
            {slow?.lapsRemaining != null && (
              <span style={{ fontFamily: theme.font.label, marginLeft: "auto", fontWeight: 600, fontSize: "0.62em", letterSpacing: "0.08em", color: t.textDim }}>{slow.lapsRemaining} LAPS LEFT</span>
            )}
          </div>
        );
      })()}
      ...
      {tyre && (
        <div style={{ display: "flex", gap: 6, marginTop: 8, alignItems: "stretch" }}>
          <span style={{ fontFamily: theme.font.label, fontSize: "0.5em", fontWeight: 600, letterSpacing: "0.1em", color: t.textDim2, alignSelf: "center", width: "3em" }}>TYRES</span>
