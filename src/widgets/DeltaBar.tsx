...
function DeltaBar({ theme, config }: BaseWidgetProps<DeltaBarConfig>) {
  const slow = useSlow();
  const delta = config.reference === "best" ? slow?.deltaBestS : slow?.deltaSessionBestS;
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", height: "1.3em", borderRadius: "0.65em", background: "rgba(255,255,255,0.07)", overflow: "hidden", isolation: "isolate" }}>
      ...
      {config.showNumeric && (
        <div style={{ textAlign: "center" }}>
          <span style={{ fontFamily: theme.font.mono, fontWeight: 700, fontSize: "2em", lineHeight: 1, color }}>{fmtDelta(delta)}</span>
          <span style={{ fontSize: "0.66em", fontWeight: 600, letterSpacing: "0.14em", color: theme.colors.textDim2, marginLeft: "0.7em" }}>
